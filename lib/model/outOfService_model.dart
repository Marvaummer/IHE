...
//
//     final outOfServiceModel = outOfServiceModelFromJson(jsonString);

import 'dart:convert';

List<OutOfServiceModel> outOfServiceModelFromJson(String str) => List<OutOfServiceModel>.from(json.decode(str).map((x) => OutOfServiceModel.fromJson(x)));

String outOfServiceModelToJson(List<OutOfServiceModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));

class OutOfServiceModel {
  String? id;
  int? vehiclEId;
  int? owneRId;
  ...
  factory OutOfServiceModel.fromJson(Map<String, dynamic> json) => OutOfServiceModel(
    id: json["\u0024id"],
