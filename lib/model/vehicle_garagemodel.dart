...
//
//     final vehiclegarageModel = vehiclegarageModelFromJson(jsonString);

import 'dart:convert';

List<VehiclegarageModel> vehiclegarageModelFromJson(String str) => List<VehiclegarageModel>.from(json.decode(str).map((x) => VehiclegarageModel.fromJson(x)));

String vehiclegarageModelToJson(List<VehiclegarageModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));

class VehiclegarageModel {
  String? id;
  int? vehiclEId;
  int? owneRId;
  ...
  factory VehiclegarageModel.fromJson(Map<String, dynamic> json) => VehiclegarageModel(
    id: json["\u0024id"],
